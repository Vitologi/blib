				...
				for(key in now){
					if(key === '0'){
						...
						if(old[key] !== now[key]){
							...
							//строим
							blib(parent.dom).html(blib.build(this.chunk[deep], undefined, parent, blocks));
							return;
						}
					...
					if(blib.is(now[key],["object","array"])){
						...
						//строим
						parent.dom.insertBefore(blib.build(this.chunk[deep+'.'+key], undefined, parent, blocks), onDelete);
